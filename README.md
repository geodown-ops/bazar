...
| `promoCode` | `String` | 使用的優惠碼（未使用為空字串）。 | `"SUMMER300"` |
| `discount` | `Number` | 優惠碼折抵金額。 | `300` |
| `totalPrice` | `Number` | 訂單總金額 (`roomPrice` + `packagePrice` − `discount`)。 | `2960` |
...
### 1-1. 優惠碼 (`promoCodes`)
儲存在 `db.json` 中的 `promoCodes` 陣列：

| 欄位名稱 (Field) | 資料類型 (Type) | 說明 (Description) | 範例 (Example) |
| :--- | :--- | :--- | :--- |
| `code` | `String` | **主鍵**，優惠碼（一律大寫，3-20 碼英數字與 `-` `_`）。 | `"SUMMER300"` |
| `type` | `String` | 折扣方式（`amount` 固定金額 \| `percent` 百分比）。 | `"amount"` |
| `value` | `Number` | 折扣數值（金額=元數；百分比=折抵 %，1-100）。 | `300` |
| `maxUses` | `Number` | 核銷次數上限（`0` = 不限）。 | `50` |
| `expiresAt` | `String` | 有效期限 YYYY-MM-DD，含當日（空字串 = 永久）。 | `"2026-08-31"` |
| `enabled` | `Boolean` | 是否啟用（後台可隨時停用/啟用）。 | `true` |
| `usedCount` | `Number` | 已核銷次數。 | `3` |
| `redemptions` | `Array` | 核銷紀錄 `{bookingId, name, amount, usedAt}`。 | — |
| `note` | `String` | 備註（活動說明）。 | `"暑期 FB 活動"` |

**核銷規則**：旅客在前台估價表單輸入優惠碼即試算折抵（`POST /api/promo/validate`，不佔名額）；成立訂單時後端重新驗證並寫入折抵；訂單**實際付款完成**（TapPay 入帳或管理員把匯款訂單改為已付款/已完成）才計入 `usedCount` 與核銷紀錄，且每筆訂單只核銷一次（冪等）。折抵上限為訂單金額 − 1 元，確保信用卡仍有最低請款金額。
...
*   **房價與套裝行程試算表單 (Booking Calculator)**：
    ...
    *   **優惠碼折抵**：明細卡內可輸入優惠碼並「套用/移除」，即時顯示折抵金額與折後總價；送出訂單時後端會重新驗證（無效即擋下），付款頁與確認信亦顯示折抵明細。
...
*   **優惠碼管理**：建立優惠碼（固定金額或百分比折抵、使用次數上限、有效期限、備註）、隨時停用/啟用/刪除；清單顯示「已核銷 / 上限」與狀態（啟用中/已停用/已過期/已用完），「核銷紀錄」可查看每筆折抵的訂單編號、旅客、金額與時間。
*   **系統設定**：
